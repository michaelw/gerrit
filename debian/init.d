...
# Configuration variables.  These may be set in /etc/default/ins-gerrit.
#
# GERRIT_SITE
...
##################################################
if test -f /etc/default/ins-gerrit ; then
  . /etc/default/ins-gerrit
fi
...
# Installation/update
##################################################
# delay initialization until we are ready to start
if [ "$ACTION" = "start" ]; then
    if [ "$NO_START" = 1 ]; then
        echo "Not starting gerrit - NO_START=1 in /etc/default/ins-gerrit"
        exit 0
    fi

    if ! [ -d /opt/ins-gerrit/etc/ssh_host_key ]; then
        [ -d $GERRIT_SITE ] || mkdir -p $GERRIT_SITE
        su -l $GERRIT_USER --shell=/bin/sh -c \
            "$JAVA -jar $GERRIT_WAR init -d ${GERRIT_SITE} --install-all-plugins --batch --no-auto-start" || true
        if test -f $GERRIT_SITE/bin/gerrit.sh; then
            mv $GERRIT_SITE/bin/gerrit.sh $GERRIT_SITE/bin/gerrit.sh.DISARM.$(date -Iseconds)
        fi
    fi
fi
